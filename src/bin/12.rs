...
impl Pos {
  fn neighbours(&self, all_positions: &Vec<(usize, usize, char)>, part2: bool) -> Vec<Pos> {
    ...
    let valid_moves = possible_moves.iter().filter_map(| (x_diff, y_diff)| {
        if x_diff < &0 {
    ...
    let valid = all_positions.iter().filter_map(|position| {
...
pub fn part_one(input: &str) -> Option<usize> {
    ...
    let path = bfs(&start, |p| p.neighbours(&all_positions, false), |p| p == &goal).expect("Should have goal");
    Some(path.len() - 1)
}
...
pub fn part_two(input: &str) -> Option<usize> {
    ...
    let path = bfs(&start, |p| p.neighbours(&all_positions, true), |p| p.2 == 'a').expect("Should have goal");
    Some(path.len() - 1)
}
