...
pub fn part_one(input: &str) -> Option<u32> {
    let mut rock_structures: HashSet<(u32, u32)> = input.lines().flat_map(|line| {
        line.split(" -> ").map(|rock_struct| {
            let (x, y) = rock_struct.split_once(",").unwrap();
            (x.parse::<u32>().unwrap(), y.parse::<u32>().unwrap())
        }).collect::<Vec<(u32, u32)>>().windows(2).flat_map(| window | {
            let from = window[0];
            let to = window[1];
            if from.0 == to.0 { //Same X
                if from.1 > to.1 {
                    (to.1..=from.1).map(|y| (from.0, y)).collect::<Vec<(u32, u32)>>()
                } else {
                    (from.1..=to.1).map(|y| (from.0, y)).collect::<Vec<(u32, u32)>>()
                }
            } else {
                if from.0 > to.0 {
                    (to.0..=from.0).map(|x| (x, from.1)).collect::<Vec<(u32, u32)>>()
                } else {
                    (from.0..=to.0).map(|x| (x, from.1)).collect::<Vec<(u32, u32)>>()
                }
            }
        }).collect::<Vec<(u32, u32)>>()
    }).collect();
