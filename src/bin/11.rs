...
impl Operation {
    pub fn execute_operation<'a>(&self, item: u64) -> u64 {
        match self {
            Operation::Add(str) => {
                if str == &"old".to_owned() { return item + item;}
                else {
...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    for line in input.lines().array_chunks::<7>() {
        let id = line[0].trim().split(" ").last().unwrap().replace(":", "").parse::<u64>().unwrap();
        let items = line[1].trim().split(": ").last().unwrap().split(", ").map(|item| item.parse::<u64>().unwrap()).collect::<Vec<u64>>();
        
        let op = line[2].trim().split(" ").skip(4).take(2).collect::<Vec<&str>>();
        let operation = if op[0] == "+" {
            Operation::Add(op[1].to_string())
        } else {
            Operation::Multiple(op[1].to_string())
        };
        let divisible = line[3].trim().split(" ").last().unwrap().parse::<u64>().unwrap();
        let true_id = line[4].trim().split(" ").last().unwrap().parse::<u64>().unwrap();
        let false_id = line[5].trim().split(" ").last().unwrap().parse::<u64>().unwrap();
        ...
        for i in 0..monkeys.len() {
            ...
            for item in monkey_items.clone().get(&monkey.id).unwrap() {
                ...
                if item % monkey.divisible == 0 {
                    monkey_items.entry(monkey.true_id).and_modify(|items| items.push(item));
                }
                else {
                    monkey_items.entry(monkey.false_id).and_modify(|items| items.push(item));
                }
    ...
    Some(monkeys.iter().take(2).map(|m| m.inspections).fold(1, |acc, i| acc * i ))
}
